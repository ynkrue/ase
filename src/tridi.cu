...
__global__ void dc_extract_z_kernel(const double *__restrict__ Q, long ldq, int m, int n1,
...
__global__ void dc_gather_cols_kernel(const double *__restrict__ Q, long ldq,
                                      double *__restrict__ out, long ldo, int rows, int cols,
...
__global__ void dc_set_one_kernel(double *q) {
    *q = 1.0;
}
...
// Host-side helpers
// -----------------------------------------------------------------------------
void lap_stedc(const char *compz, int n, double *d, double *e, double *Z, int ldz, double *work,
               int lwork, int *iwork, int liwork, int *info) {
    dstedc_(compz, &n, d, e, Z, &ldz, work, &lwork, iwork, &liwork, info);
}

void lap_lamrg(int n1, int n2, const double *a, int d1, int d2, int *index) {
...
// -----------------------------------------------------------------------------
DcMerge host_laed2(int m, int n1, double &rho, double *d, double *z, int *indxq, double *dlamda,
                   double *w, int *indx, int *ixc, int *ij, double *cs, int *iwk) {
...
// -----------------------------------------------------------------------------
void merge_gpu(AseHandle *ws, double *Qb, int ldq, int m, int n1, double rho, double *d, int *indxq,
               double *S, int *iwk) {
    ...
    const DcMerge r = host_laed2(m, n1, rho, d, ws->h_z, indxq, ws->h_dlamda, ws->h_w, ws->h_indx,
                                 ws->h_ixc, ws->h_ij, ws->h_cs, iwk);
    const int k = r.k;
    double *Q2 = ws->M;
    const int ldq2 = m;
    ...
    if (n23)
        cublas::gemm(ws, CUBLAS_OP_N, CUBLAS_OP_N, n2, k, n23, &one,
                     Q2 + n1 + (size_t)r.ctot[0] * ldq2, ldq2, S + r.ctot[0], k, &zero, Qb + n1,
                     ldq);
    else
        CUDA_CHECK(cudaMemset2DAsync(Qb + n1, (size_t)ldq * es, 0, n2 * es, k, st));
    if (n12)
        cublas::gemm(ws, CUBLAS_OP_N, CUBLAS_OP_N, n1, k, n12, &one, Q2, ldq2, S, k, &zero, Qb,
                     ldq);
    else
...
void solve_leaves(AseHandle *ws, int subpbs, const int *part, double *d, double *e, double *Qb,
                  int ldq, int *indxq) {
...
#pragma omp parallel for schedule(dynamic) reduction(+ : nfail)
    ...
    for (int i = 0; i < subpbs; ++i) {
        const int sm = (i == 0) ? 0 : part[i - 1];
        const int ms = (i == 0) ? part[0] : part[i] - part[i - 1];
        CUDA_CHECK(cudaMemcpy2DAsync(Qb + sm + (size_t)sm * ldq, (size_t)ldq * sizeof(double),
                                     &hQ[qoff[i]], (size_t)ms * sizeof(double), ms * sizeof(double),
                                     ms, cudaMemcpyHostToDevice, ws->stream));
    }
}
...
// -----------------------------------------------------------------------------
void laex0_gpu(AseHandle *ws, int bn, double *d, double *e, double *Qb, int ldq, double *S,
               int *indxq, int *iwk, int *part) {
    part[0] = bn;
    int subpbs = 1;
    ...
    // final ascending order: permute d on the host, gather Q columns on the device
    {
        ...
        dc_gather_cols_kernel<<<g, 256, 0, ws->stream>>>(Qb, (long)ldq, ws->M, (long)bn, bn, bn,
                                                         ws->dc_indx);
        CUDA_CHECK(cudaMemcpy2DAsync(Qb, (size_t)ldq * sizeof(double), ws->M,
                                     (size_t)bn * sizeof(double), bn * sizeof(double), bn,
...
void stedx(AseHandle *ws, int n, double *d, double *e, double *Q, int ldq, double *S) {
    ...
    while (start < n) {
        ...
        if (bn != 0.0) {
            ...
            if (m > DC_LEAF) {
                laex0_gpu(ws, m, &d[start], &e[start], Qb, ldq, S, &indxq[start], ws->h_iwk,
                          ws->h_part);
            } else {
                // single block, so it takes the first slice of the leaf workspaces
                int info = 0;
                lap_stedc("I", m, &d[start], &e[start], ws->h_leafQ, m, ws->h_leafwork,
                          1 + 4 * m + m * m, ws->h_leafiwork, 3 + 5 * m, &info);
                if (info != 0) fail("stedc (block)", info);
                CUDA_CHECK(cudaMemcpy2DAsync(Qb, (size_t)ldq * sizeof(double), ws->h_leafQ,
                                             (size_t)m * sizeof(double), m * sizeof(double), m,
                                             cudaMemcpyHostToDevice, ws->stream));
    ...
    if (nblocks > 1) {
        ...
        for (int i = 0; i < n; ++i)
            dt[i] = d[perm[i]];
        std::copy(dt, dt + n, d);
        CUDA_CHECK(cudaMemcpyAsync(ws->dc_indx, perm, n * sizeof(int), cudaMemcpyHostToDevice,
                                   ws->stream));
        const dim3 g(div_up(n, 256), std::min(n, 4096));
        dc_gather_cols_kernel<<<g, 256, 0, ws->stream>>>(Q, (long)ldq, ws->M, (long)n, n, n,
                                                         ws->dc_indx);
        CUDA_CHECK(cudaMemcpy2DAsync(Q, (size_t)ldq * sizeof(double), ws->M,
                                     (size_t)n * sizeof(double), n * sizeof(double), n,
                                     cudaMemcpyDeviceToDevice, ws->stream));
    }
}
...
void tridi_dc(AseHandle *ws, double *d, double *e, double *eval, double *evec, double *scratch) {
    ...
    if (n > 1)
        CUDA_CHECK(cudaMemcpyAsync(ws->h_e, e, (n - 1) * sizeof(double), cudaMemcpyDeviceToHost,
                                   ws->stream));
    CUDA_CHECK(cudaStreamSynchronize(ws->stream));
    ...
    CUDA_CHECK(
        cudaMemcpyAsync(eval, ws->h_d, n * sizeof(double), cudaMemcpyHostToDevice, ws->stream));
}
