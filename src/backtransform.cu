...
template<int RED>
__device__ __forceinline__ double bt_reduce(double v)
{
#pragma unroll
...
__device__ __forceinline__ void bt_pass(double* __restrict__ Mc, long ldm, long baseRow, const double* __restrict__ sU,
                                        double* __restrict__ sHead, double* __restrict__ sDone, int lane)
{
...
__global__ void bc_back_kernel(int n, int cols, int extra, int nsweeps, int lastU, const double* __restrict__ U,
                               long ldu, double* __restrict__ M, long ldm)
{
    constexpr int     WIN = PT * 32;
...
#pragma unroll
                    for (int t = 0; t < PT; t++)
                        sU[k * WIN + lane + t * 32] = __ldcs(&U[(uOff + k) * ldu + baseRow + 1 + k + lane * PT + t]);
                }
                else
                {
#pragma unroll
...
void sbr_back(AseHandle* ws, const double* Y, const double* W, double* M)
{
    ...
    for (int j = jmax; j >= 0; j -= b)
    {
        ...
        cublas::gemm(ws, CUBLAS_OP_T, CUBLAS_OP_N, b, n, rows, &one, Yp, lda, Mb, ldm, &zero, ws->Z, b);
        cublas::gemm(ws, CUBLAS_OP_N, CUBLAS_OP_N, rows, n, b, &neg1, Wp, lda, ws->Z, b, &one, Mb, ldm);
    }
}
...
void back_transform(AseHandle* ws, const double* Y, const double* W, const double* U, double* evec)
{
    ...
    CUDA_CHECK(cudaMemcpy2DAsync(M, ldm * sizeof(double), evec, lda * sizeof(double), n * sizeof(double), n,
