...
// -----------------------------------------------------------------------------
__device__ void dev_laed6(int kniter, bool orgati, double rho, const double* d, const double* z, double finit,
                          double& tau, int& info)
{
    ...
    if (kniter == 2)
    {
        ...
        if (d[0] == tau || d[1] == tau || d[2] == tau) { tau = zero; }
        else
        {
    ...
    double temp   = orgati ? min(fabs(d[1] - tau), fabs(d[2] - tau)) : min(fabs(d[0] - tau), fabs(d[1] - tau));
    bool   scale  = false;
    double sclinv = one;
    ...
    if (fabs(f) > zero)
    {
        ...
        {
            double temp1, temp2;
            if (orgati) { temp1 = dscale[1] - tau, temp2 = dscale[2] - tau; }
            else
            {
...
// -----------------------------------------------------------------------------
__device__ void dev_laed5(int i, const double* d, const double* z, double* delta, double rho, double& dlam)
{
...
// -----------------------------------------------------------------------------
__device__ void dev_laed4(int n, int i, const double* __restrict__ D, const double* __restrict__ Z, double rho,
                          double& tau_out, int& org_out, double& dlam, int& info)
{
    ...
    if (i == n)
    {
        ...
        for (int niter = 3; niter <= MAXIT; ++niter)
        {
            ...
            if (w * eta > zero) eta = -w / (dpsi + dphi);
            double temp = tau + eta;
            if (temp > dltub || temp < dltlb) eta = (w < zero) ? (dltub - tau) / two : (dltlb - tau) / two;
            tau += eta;
            evalw(tau);
            w = rhoinv + phi + psi;
        }
    ...
    double eta;
    {
        if (!swtch3)
        {
            ...
            else if (a <= zero) { eta = (a - sqrt(fabs(a * a - four * b * c2))) / (two * c2); }
            else
            {
                eta = two * b / (a + sqrt(fabs(a * a - four * b * c2)));
            }
        ...
        double tmp2 = tau + eta;
        if (tmp2 > dltub || tmp2 < dltlb) eta = (w < zero) ? (dltub - tau) / two : (dltlb - tau) / two;
    }
 ...
 */
__global__ void dc_laed4_kernel(int k, const double* __restrict__ dlamda, const double* __restrict__ w, double rho,
                                double* delta, double* dtau, int* dorg, double* dlam, int* info)
{
...
__global__ void dc_fill_delta_kernel(int k, const double* __restrict__ dlamda, const double* __restrict__ dtau,
                                     const int* __restrict__ dorg, double* __restrict__ delta)
{
...
__global__ void dc_weights_kernel(int k, const double* __restrict__ dlamda, const double* __restrict__ delta,
                                  const double* __restrict__ w_in, double* __restrict__ w_out)
{
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= k) return;
    const double di = dlamda[i];
 ...
 */
template<int NT>
__global__ void dc_form_s_kernel(int k, const double* __restrict__ delta, const double* __restrict__ w,
                                 const int* __restrict__ ixc, double* __restrict__ S, int normalize)
{
...
void dc_secular_solve(AseHandle* ws, int k, double rho, double* S)
{
    ...
    dc_laed4_kernel<<<div_up(k, 128), 128, 0, st>>>(k, ws->dc_dlamda, ws->dc_w, rho, ws->Sdc, ws->dc_tau, ws->dc_org,
                                                    ws->dc_lam, ws->dc_info);
    if (k > 2)
    {
        const dim3 g(div_up(k, 256), std::min(k, 1024));
        dc_fill_delta_kernel<<<g, 256, 0, st>>>(k, ws->dc_dlamda, ws->dc_tau, ws->dc_org, ws->Sdc);
        dc_weights_kernel<<<div_up(k, 256), 256, 0, st>>>(k, ws->dc_dlamda, ws->Sdc, ws->dc_w, ws->dc_wt);
    }
